...
export const PropiedadesRepository = {
  ...
  async listar(filtros: FiltrosPropiedades = {}): Promise<Propiedad[]> {
    ...
    if (error) { console.error('PropiedadesRepository.listar:', error); return [] }
    return data || []
  },
  ...
  async listarDestacadas(limite = 3): Promise<Propiedad[]> {
    ...
    if (error) { console.error('PropiedadesRepository.listarDestacadas:', error); return [] }
    return data || []
  },
  ...
  async obtenerPorId(id: string): Promise<Propiedad | null> {
    ...
    if (error) { console.error('PropiedadesRepository.obtenerPorId:', error); return null }
    return data
  },
  ...
  async crear(datos: Omit<Propiedad, 'id' | 'created_at' | 'updated_at'>): Promise<{ data: Propiedad | null; error: string | null }> {
    const supabase = await createClient()
    const { data, error } = await supabase
      .from('propiedades')
      .insert(datos)
  ...
  async actualizar(id: string, datos: Partial<Propiedad>): Promise<{ error: string | null }> {
    const supabase = await createClient()
